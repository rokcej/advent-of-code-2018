...
while not search.empty():
	...
	if crucible.position == end_position and crucible.direction_steps >= MIN_FORWARD_STEPS:
		print(heat_loss)
		break
	...
	# Optimization: eliminate states that can't possibly be better
	if crucible.direction_steps >= MIN_FORWARD_STEPS:
		for steps in range(crucible.direction_steps, MAX_FORWARD_STEPS + 1):
			visited.add((crucible.position, crucible.direction, steps))
