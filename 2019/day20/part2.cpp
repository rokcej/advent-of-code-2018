...
void copy(const std::vector<std::string>& src, std::vector<std::string>& dest) {
...
void disp(const std::vector<std::string>& map) {
...
int main() {
	...
	bfs.emplace(x_start, y_start, 0);
	int steps = 0;
	while (!bfs.empty()) {
		for (int count = bfs.size(); count > 0; --count) {
			auto [x, y, level] = bfs.front();
			bfs.pop();

			if (x == x_goal && y == y_goal && level == 0) {
			...
			if (it != portals.end()) {
				...
				int level2 = level;
				if (x == x0_out || x == x0_out + w_out - 1 || y == y0_out || y == y0_out + h_out - 1) {
					--level2;
				} else {
					++level2;
				}
				...
				if (level2 < 0) {
					continue;
				} else if (level2 >= maps.size()) {
					maps.emplace_back();
					copy(map, maps[level2]);
				}

				if (maps[level2][y2][x2] == '.') {
					maps[level2][y2][x2] == '@';
					bfs.emplace(x2, y2, level2);
					continue;
				}
			}
