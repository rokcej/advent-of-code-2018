...
int main() {
	...
	int x = 0, y = 0, dir = 0;
	map[0][0] = 1; // Start on a white panel
	...
	for (const auto& it : map) {
		if (it.first < yMin) yMin = it.first;
		else if (it.first > yMax) yMax = it.first;
		for (const auto& jt : it.second) {
			if (jt.first < xMin) xMin = jt.first;
			else if (jt.first > xMax) xMax = jt.first;
		}
	}

	for (int row = yMax; row >= yMin; --row) { // It's upside down
		for (int col = xMin; col <= xMax; ++col) {
			int color = 0;
			if (map.find(row) != map.end() && map[row].find(col) != map[row].end())
				color = map[row][col];
			std::cout << (char)(color == 1 ? '#' : ' ');
		}
		std::cout << std::endl;
	}
